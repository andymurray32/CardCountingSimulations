...
// Description: Balanced, 'Level- 3' method with only one parameter (True count). Utilizes
// a betting strategy based on the true count


public class Halves implements Player {
    ...
    public Halves(int chips, int playerIndex, int minBet) {
        this.chips = chips;
    ...
    public void newShuffle() {
        this.runningCount = 0;
        this.runningCount2= 0; //for optimal
        this.trueCount=0;
        this.trueCount2=0;
    ...
    public void printPlayer(int chips) {
        ...
        System.out.println("Player " + this.playerIndex() + ": Halves"+ "\n\tRemaining Chips: " + (this.chips)+ "\n\tLoss or gain:"+ (net)+"\n\tMissed oppurtunities:"+missedOP+"\n\tTimes betted more than minimum:"+betExtra) ;
    }

    public void printBet() {
        System.out.println("Wong Halves Counter- Player:" + this.playerIndex() + " |Bet: " + bet+" |Count = " + this.runningCount+ "|True Count ="+ trueCount);
    }
    ...
    public void updateRunningCount(ArrayList<String> countList, int decksize) {
        ...
        if (b <=0){
           ...
           // System.out.println("RUNNINGCOUNT2 "+runningCount2+"  B"+b);
            //System.out.println("RUNNINGCOUNT "+runningCount+"  B"+b);
            this.trueCount2= runningCount2/b;
        }
    }
}
