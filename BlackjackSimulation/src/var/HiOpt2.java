...
// Description: Balanced Level 2 method with a seperate count for aces whilst betting
//utilizing true count
...
public class HiOpt2 implements Player {
    ...
    public HiOpt2(int chips, int playerIndex, int minBet) {
        this.chips = chips;
...
//basic strategy
    ...
    public void printPlayer(int chips) {
        ...
        System.out.println("Player " + this.playerIndex() + ": HiOpt2"+ "\n\tRemaining Chips: " + (this.chips)+ "\n\tLoss or gain:"+ (net)+"\n\tMissed oppurtunities:"+missedOP+"\n\tTimes betted more than minimum:"+betExtra) ;
    }

    public void printBet() {
        System.out.println("HiOpt2- Player:" + this.playerIndex() + " |Bet: " + bet+" |Count = " + this.runningCount+ "|True Count ="+ trueCount);
    }
    ...
    public void updateRunningCount(ArrayList<String> countList, int decksize) {
        ...
        if (b <=0){
            ...
            if (b>2){
                if (a>4) {
                    System.out.println("True count increased for HiOpt2-Shoe rich in aces");
                    trueCount++;
